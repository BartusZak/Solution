...
import {
  validatorsFunctions,
  checkFormContainErrors,
  createSlotFunctionsNames,
  runSingleValidation,
  runOnSubmitValidation
} from './index';
...
class FancyForm extends React.PureComponent {
  ...
  detectInitialValuesChanges = initialValues => {
    ...
    formKeys.forEach(key => {
      if (oldValues[key] !== values[key])
        errors[key] = runSingleValidation(
          values[key],
          settings[key].validators,
          settings[key].label
        );
      if (errors[key]) isFormInvalid = true;
    });
  ...
  putChanges = (value, key) => {
    ...
    if (isFormDirty)
      this.setState({
        values,
        errors,
        isFormInvalid: checkFormContainErrors(errors)
      });
  ...
  handleChangeValues = values => {
    this.detectInitialValuesChanges(values);
  };

  checkSubmitedData = () => {
    const { values, formKeys, settings } = this.state;
    const { errors, isFormInvalid } = runOnSubmitValidation(
      values,
      formKeys,
      settings
    );
      ...
      case 'date-picker':
        return (
          <DatePicker
            ...
            {...settings.componentProps}
            onChange={date => this.handleDateChange(date, props.key)}
          ...
          <form className={formClass} onSubmit={this.handleSubmit}>
            {formKeys.map(key => {
              if (settings[key].needsSlot && this.props[this.slots[key]])
                return this.props[this.slots[key]](
                  key,
                  values,
                  this.handleChangeFromEvent,
                  settings[key],
                ...
                return (
                  <section className={inputWrapperClass} key={key}>
                    <label className={labelClass}>
                      {settings[key].label}{' '}
                      {settings[key].validators
                        ? settings[key].validators.required
                          ? '*'
                          : null
                        : null}
                    </label>
                    ...
                    {this.renderComponent(settings[key], {
                      value: values[key],
                      onChange: e => this.handleChangeFromEvent(e, key),
                      key
                    })}
            ...
            {renderSubmitBtn ? (
              renderSubmitBtn(this.handleSubmit)
            ) : (
              <Button
                title={btnTitle}
                mainClass={btnClass}
                type="submit"
                disable={isFormInvalid || isSubmiting}
              />
            )}
          </form>
        )}
