...
const ResultBlock = ({
  ...
  let errorObjectsList =
    errorObjects.length > 0
      ? errorObjects.map((error, i) => {
          let key = error.errors ? Object.keys(error.errors) : null;
          if (error.errors) {
            return (
              <li key={i}>
  ...
  if (errorStatus !== null || errorStatus !== undefined) {
    if (errorStatus !== 200) {
      type === 'modalInParent'
        ? classes.push('modal-result-error')
      ...
      type === 'modalInParent'
        ? classes.push('modal-result-success')
  ...
  return (
    <div className={classes.join(' ')}>
      {shouldRender(errorOnly, errorStatus) && errorStatusContent}
    </div>
  );
};
